"""MARD standard-range screen colors used by the MVP matcher.

The values are display approximations for the A-H and M standard series.
Physical-bead calibration is intentionally outside the MVP scope.
"""
...
def _make_palette() -> tuple[MardColor, ...]:
    colors: list[MardColor] = []
    for series, values in _SERIES.items():
        for number, value in enumerate(values.split(), start=1):
            colors.append(
                MardColor(
                    code=f"{series}{number}",
                    hex=f"#{value}",
                    rgb=tuple(int(value[offset : offset + 2], 16) for offset in (0, 2, 4)),
                )
            )
    return tuple(colors)


MARD_STANDARD_PALETTE = _make_palette()
